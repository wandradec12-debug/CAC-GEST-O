...
{detail&&<div className="overlay"><section className="modal detailModal"><div className="modalhead"><div><h2>{detail.nome_completo||detail.nome||detail.razao_social}</h2><p>Perfil do cliente</p></div><button className="close" onClick={()=>setDetail(null)}>×</button></div><div className="detailGrid">{[['Código',detail.codigo],['CPF/CNPJ',detail.cpf_cnpj],['Nascimento',formatDeclaracaoDate(detail.nascimento||detail.data_nascimento||detail.dataNascimento||detail.data_de_nascimento)],['Naturalidade',detail.naturalidade],['Filiação - Pai',detail.filiacao_pai],['Filiação - Mãe',detail.filiacao_mae],['Estado civil',detail.estado_civil],['RG',detail.rg],['Órgão RG',detail.rg_orgao],['Tipo',detail.tipo_cliente],['Status',detail.status],['Telefone',detail.telefone],['WhatsApp',detail.whatsapp],['E-mail',detail.email],['Endereço',[detail.endereco,detail.numero,detail.complemento].filter(Boolean).join(', ')],['Cidade',[detail.cidade,detail.estado].filter(Boolean).join(' - ')],['Categoria CAC',detail.categoria_cac],['Número do CR',detail.cr_numero],['Validade do CR',detail.cr_validade],['Situação do CR',detail.cr_situacao],['Órgão emissor',detail.cr_orgao],['Responsável',detail.responsavel],['Origem do cadastro',detail.origem_cadastro]].map(([l,v])=><div className="detailItem" key={l}><span>{l}</span><b>{v||'—'}</b></div>)}</div><div className="detailObs"><span>Observações</span><p>{detail.observacoes||'—'}</p></div><div className="actions"><button className="secondary" onClick={()=>setDetail(null)}>Fechar</button><button type="button" onClick={()=>{setDeclaracao({...detail});setDetail(null);}}>Gerar declaração</button><button onClick={()=>{setForm({...empty,...detail,nascimento:formatDateInput(detail.nascimento),cr_emissao:formatDateInput(detail.cr_emissao),cr_validade:formatDateInput(detail.cr_validade)});setEditing(true);setDetail(null);setShow(true)}}>Editar cliente</button></div></section></div>}{declaracao&&<div className="overlay declarationOverlay"><section className="modal declarationModal"><div className="modalhead noPrint"><div><h2>Declaração</h2><p>Pré-visualização para impressão</p></div><button type="button" className="close" onClick={()=>setDeclaracao(null)}>×</button></div><div className="declarationPaper printDeclaration">
<h1>DECLARAÇÃO DE INEXISTÊNCIA DE INQUÉRITOS POLICIAIS OU<br/>PROCESSOS CRIMINAIS</h1>
<p>Eu, <strong>{String(declaracao.nome_completo||declaracao.razao_social||declaracao.nome||'').toUpperCase()}</strong>, BRASILEIRO, natural de <strong>{String(declaracao.naturalidade||'').toUpperCase()}</strong>, nascido em <strong>{getNascimento(declaracao)||'NÃO INFORMADO'}</strong>, filho(a) de <strong>{String(declaracao.filiacao_pai||'').toUpperCase()}</strong> e <strong>{String(declaracao.filiacao_mae||'').toUpperCase()}</strong>, <strong>{String(declaracao.estado_civil||'').toUpperCase()}</strong> residência no(a), <strong>{[declaracao.endereco,declaracao.numero,declaracao.complemento,declaracao.bairro].filter(Boolean).join(', ')}</strong>, CEP <strong>{declaracao.cep||''}</strong>, {String(declaracao.cidade||'').toUpperCase()} - {String(declaracao.estado||'').toUpperCase()} RG nº <strong>{declaracao.rg||''}</strong>, declaro, sob as penas da lei, que não respondo a inquéritos policiais nem a processos criminais no estado de domicílio e nos entes federativos, e estou ciente de que, em caso de falsidade ideológica, ficarei sujeito às sanções prescritas no Código Penal e às demais cominações legais aplicáveis.</p>
...
<p className="assinatura"><strong>{String(declaracao.nome_completo||declaracao.razao_social||declaracao.nome||'').toUpperCase()}</strong></p>
<p className="cpf">CPF Nº {declaracao.cpf_cnpj||''}</p>
</div><div className="actions noPrint"><button type="button" className="secondary" onClick={()=>setDeclaracao(null)}>Fechar</button><button type="button" onClick={()=>window.print()}>Imprimir / Salvar PDF</button></div></section></div>}{show&&<div className="overlay"><section className="modal"><div className="modalhead"><div><h2>{editing?'Editar cliente':'Novo cliente'}</h2><p>Cadastro completo do cliente</p></div><button className="close" onClick={()=>{setShow(false);setEditing(false)}}>×</button></div><form onSubmit={save}>
