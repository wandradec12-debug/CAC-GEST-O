...
const validDateParts=(p)=>{
  if(!p)return null;
  const dt=new Date(Date.UTC(p.y,p.m-1,p.d));
  return dt.getUTCFullYear()===p.y&&dt.getUTCMonth()===p.m-1&&dt.getUTCDate()===p.d?p:null;
};
const normalizeNascimento=(value)=>{
  const p=validDateParts(parseDateParts(value));
...
app.post('/api/usuarios',auth,adminOnly,async(req,res)=>{try{const email=String(req.body.email||'').trim().toLowerCase(),role=String(req.body.role||'operacional'),senha=String(req.body.senha||'');if(!email||!email.includes('@'))return res.status(400).json({error:'Informe um e-mail válido'});if(senha.length<6)return res.status(400).json({error:'A senha deve ter pelo menos 6 caracteres'});if(!['administrador','gerente','consultor','operacional'].includes(role))return res.status(400).json({error:'Perfil de usuário inválido'});const hash=await bcrypt.hash(senha,10);const r=await pool.query('INSERT INTO users(email,password_hash,role,active) VALUES($1,$2,$3,TRUE) RETURNING id,email,role,active,created_at',[email,hash,role]);await pool.query('INSERT INTO audit_logs(user_id,acao,entidade,entidade_id,detalhes) VALUES($1,$2,$3,$4,$5)',[req.user.id,'criar','usuario',r.rows[0].id,{email,role}]);res.status(201).json(r.rows[0])}catch(e){if(e.code==='23505')return res.status(409).json({error:'E-mail já cadastrado'});res.status(500).json({error:'Erro ao cadastrar usuário'})}});
app.put('/api/usuarios/:id',auth,adminOnly,async(req,res)=>{try{const id=Number(req.params.id),role=String(req.body.role||'operacional'),active=req.body.active!==false;if(!['administrador','gerente','consultor','operacional'].includes(role))return res.status(400).json({error:'Perfil de usuário inválido'});if(id===Number(req.user.id)&&!active)return res.status(400).json({error:'Você não pode desativar o próprio usuário'});const r=await pool.query('UPDATE users SET role=$1,active=$2 WHERE id=$3 RETURNING id,email,role,active,created_at',[role,active,id]);if(!r.rowCount)return res.status(404).json({error:'Usuário não encontrado'});await pool.query('INSERT INTO audit_logs(user_id,acao,entidade,entidade_id,detalhes) VALUES($1,$2,$3,$4,$5)',[req.user.id,'editar','usuario',id,{role,active}]);res.json(r.rows[0])}catch(e){res.status(500).json({error:'Erro ao atualizar usuário'})}});
app.put('/api/usuarios/:id/senha',auth,adminOnly,async(req,res)=>{const id=Number(req.params.id),senha=String(req.body.senha||'');if(senha.length<6)return res.status(400).json({error:'A senha deve ter pelo menos 6 caracteres'});const hash=await bcrypt.hash(senha,10);const r=await pool.query('UPDATE users SET password_hash=$1 WHERE id=$2 RETURNING id,email',[hash,id]);if(!r.rowCount)return res.status(404).json({error:'Usuário não encontrado'});await pool.query('INSERT INTO audit_logs(user_id,acao,entidade,entidade_id,detalhes) VALUES($1,$2,$3,$4,$5)',[req.user.id,'alterar_senha','usuario',id,{email:r.rows[0].email}]);res.json({ok:true,message:'Senha alterada com sucesso'});});
...
app.get('/api/clientes',auth,async(req,res)=>{const r=await pool.query('SELECT * FROM clientes ORDER BY id DESC');res.json(r.rows.map(formatCliente))});
...
app.put('/api/clientes/:id',auth,roles('administrador','gerente','consultor'),async(req,res)=>{try{
const id=Number(req.params.id),b=req.body;
const q=`UPDATE clientes SET codigo=$1,nome=$2,razao_social=$3,nome_fantasia=$4,nome_completo=$5,cpf_cnpj=$6,nascimento=$7,naturalidade=$8,filiacao_pai=$9,filiacao_mae=$10,estado_civil=$11,rg=$12,rg_orgao=$13,tipo_cliente=$14,email=$15,telefone=$16,whatsapp=$17,cep=$18,endereco=$19,numero=$20,complemento=$21,bairro=$22,cidade=$23,estado=$24,origem_cadastro=$25,responsavel=$26,status=$27,categoria_cac=$28,cr_numero=$29,cr_emissao=$30,cr_validade=$31,cr_situacao=$32,cr_orgao=$33,observacoes=$34,updated_at=NOW() WHERE id=$35 RETURNING *`;
const v=[b.codigo||null,b.nome||b.nome_completo||b.razao_social,b.razao_social||null,b.nome_fantasia||null,b.nome_completo||null,b.cpf_cnpj||null,normalizeNascimento(b.nascimento),b.naturalidade||null,b.filiacao_pai||null,b.filiacao_mae||null,b.estado_civil||null,b.rg||null,b.rg_orgao||null,b.tipo_cliente||null,b.email||null,b.telefone||null,b.whatsapp||null,b.cep||null,b.endereco||null,b.numero||null,b.complemento||null,b.bairro||null,b.cidade||null,b.estado||null,b.origem_cadastro||null,b.responsavel||null,b.status||'ativo',b.categoria_cac||null,b.cr_numero||null,normalizeSqlDate(b.cr_emissao),normalizeSqlDate(b.cr_validade),b.cr_situacao||null,b.cr_orgao||null,b.observacoes||null,id];
const r=await pool.query(q,v);if(!r.rowCount)return res.status(404).json({error:'Cliente não encontrado'});await pool.query('INSERT INTO audit_logs(user_id,acao,entidade,entidade_id,detalhes) VALUES($1,$2,$3,$4,$5)',[req.user.id,'editar','cliente',id,{nome:r.rows[0].nome}]);res.json(formatCliente(r.rows[0]));
}catch(e){if(e.code==='23505')return res.status(409).json({error:'CPF/CNPJ já cadastrado'});res.status(500).json({error:'Erro ao editar cliente'})}});
app.get('/api/clientes/:id',auth,async(req,res)=>{const id=Number(req.params.id);const c=await pool.query('SELECT * FROM clientes WHERE id=$1',[id]);if(!c.rowCount)return res.status(404).json({error:'Cliente não encontrado'});const[d,p]=await Promise.all([pool.query('SELECT * FROM documentos WHERE cliente_id=$1 ORDER BY validade',[id]),pool.query('SELECT * FROM compras WHERE cliente_id=$1 ORDER BY data_compra DESC',[id])]);res.json({...formatCliente(c.rows[0]),documentos:d.rows,compras:p.rows})});
app.listen(process.env.PORT||3001,'0.0.0.0',()=>console.log('CAC GESTAO API running')); // deploy sync 2026-09-24